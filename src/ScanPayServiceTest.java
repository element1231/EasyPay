...
public class ScanPayServiceTest {
	   private static ScanPayService scanPayService;
	   private static ScanPayQueryService scanPayQueryService;

	    private static ReverseService reverseService;
	    
	    //循环调用订单查询API的次数
	    private static int payQueryLoopInvokedCount = 3;

	    //每次调用撤销API的等待时间
	    private static int waitingTimeBeforeReverseServiceInvoked = 5000;
	    //每次调用订单查询API时的等待时间，因为当出现支付失败的时候，如果马上发起查询不一定就能查到结果，所以这里建议先等待一定时间再发起查询

	    private static int waitingTimeBeforePayQueryServiceInvoked = 5000;
...
//		  
        ...
        try {
			payServiceResponseString = scanPayService.request(payData);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
 ...
 * @throws Exception
 */
private static boolean doOnePayQuery(String outTradeNo) throws Exception {
 ...
 */
private static boolean doPayQueryLoop(int loopCount, String outTradeNo) throws Exception {
    //至少查询一次
    if (loopCount == 0) {
        loopCount = 1;
    }
...
private static boolean needRecallReverse = false;
 ...
 * @throws Exception
 */
private static boolean doOneReverse(String outTradeNo) throws Exception {
 ...
 * @throws InterruptedException
 */
private static void doReverseLoop(String outTradeNo) throws Exception {
    //初始化这个标记
    needRecallReverse = true;
